...
client.channels.get(client.ayarlar.kayıt).send(`\`${req.user.username}\` added  \`${sistem.username}#${sistem.discriminator}\` to the queue`)

if (client.users.has(req.user.id) === true) {
  client.users.get(req.user.id).send(`\`${sistem.username}\` was added to the queue`)
}
...
client.channels.get(client.ayarlar.kayıt).send(`\`${req.user.username}\` edited \`${sistem.username}\``)

if (client.users.has(req.user.id) === true) {
client.users.get(req.user.id).send(`\`${sistem.username}\` successfully edited`)
}
...
client.channels.get(client.ayarlar.kayıt).send(`\`${req.user.username}\` approved \`${db.fetch(`botlar.${id}.isim`)}\``)

if (client.users.has(db.fetch(`botlar.${id}.sahipid`)) === true) {
client.users.get(db.fetch(`botlar.${id}.sahipid`)).send(`\`${db.fetch(`botlar.${id}.isim`)}\` approved :tada:`)
}
...
client.channels.get(client.ayarlar.kayıt).send(`\`${req.user.username}\` Waiting \`${db.fetch(`botlar.${id}.isim`)}\` `)

if (client.users.has(db.fetch(`botlar.${id}.sahipid`)) === true) {
client.users.get(db.fetch(`botlar.${id}.sahipid`)).send(`\`${db.fetch(`botlar.${id}.isim`)}\` waiting`)
}
...
app.post("/botyonetici/reddet/:botID", checkAuth, (req, res) => {
  ...
  client.channels.get(client.ayarlar.kayıt).send(`\`${req.user.username}\` denied \`${db.fetch(`botlar.${id}.isim`)}\` \n Reason : \`${req.body['red-sebep']}\` `)
  
  if (client.users.has(db.fetch(`botlar.${id}.sahipid`)) === true) {
  client.users.get(db.fetch(`botlar.${id}.sahipid`)).send(`\`${db.fetch(`botlar.${id}.isim`)}\` denied \n Reason : \`${req.body['red-sebep']}\``)
  }
